...
export const resolvers = {
  Query: {
    getUserPosts: async (_: any, args: any, context: any) => {
      ...
      const posts = await Promise.all(
        user.posts.map(async (postId: string) => {
    ...
    getUserComments: async (_: any, args: any, context: any) => {
      ...
      const comments = await Promise.all(
        user.comments.map(async (commentId: string) => {
  ...
  Mutation: {
    createUser: async (_: any, args: any, context: any) => {
      ...
      if (user) {
        throw new Error(
          "User already exist with this email, try different email"
        );
      }
    ...
    createPost: async (_: any, args: any, context: any) => {
      ...
      await context.models.User.findByIdAndUpdate(context.user.id, {
        $push: { posts: post._id },
      });
    ...
    updatePost: async (_: any, args: any, context: any) => {
      ...
      const updatedPost = await context.models.Post.findByIdAndUpdate(
        input.postId,
        { tweet: input.tweet },
        { new: true }
      );
    ...
    createComment: async (_: any, args: any, context: any) => {
      ...
      await context.models.Post.findByIdAndUpdate(input.postId, {
    ...
    updateComment: async (_: any, args: any, context: any) => {
      ...
      const updatedComment = await context.models.Comment.findByIdAndUpdate(
        input.commentId,
    ...
    deleteComment: async (_: any, args: any, context: any) => {
      ...
      await context.models.Post.findByIdAndUpdate(postId, {
        $pull: { comments: commentId },
      });
    ...
    followUser: async (_: any, args: any, context: any) => {
      ...
      await context.models.User.findByIdAndUpdate(userId, {
        $push: { followers: context.user.id },
      });
